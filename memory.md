...
- v0.29.4 (2026-09-26) — stato del programma: interfaccia rivista con giro sulle schede (titoli fissi, barre divise al posto delle ciambelle, schede AP compatte, modifica in Configurazione con pannellino ancorato); modelli Wi-Fi 7 («BE») trattati come Wi-Fi 6 in `capabilities.generation`; README con la lista degli AP compatibili (provati: NWA50AX PRO fw 7.12, WAC6103D-I e NWA1123-AC PRO fw 6.28; gli altri NWA/WAC/WAX/WBE sono "dovrebbero funzionare").
- Sul campo, 26/09: canali 2.4 GHz portati a 1/6/11 con la prova controllata (occupazione da 95% a ~20%); GARAGE scesa da 80% a 16% dopo `reject-legacy-station`, attivato per errore da un Esplora e non salvato: sparisce al riavvio. Da impostare anche in Nebula per renderlo stabile.
- v0.29.1 (2026-09-26): README principale in inglese + `README.it.md`; screenshot in `docs/screenshots` rifatti con `scripts/demo_data.py` + `ZM_DEMO=1` + `scripts/screenshots.py` (Playwright con Edge), mai dalla rete reale: il repo è pubblico.
- v0.27.0 (2026-09-26): ruolo utente in `users.role` (admin|viewer), il blocco delle scritture è un middleware in `main.py` (eccezioni in `VIEWER_WRITES`); dispositivi importanti in `devices.critical`, avvisati in `settings.alert_critical_down` salvato solo dopo l'invio; backup notturno con l'API di backup di SQLite in `BACKUP_DIR` (default `data/backups`). La PWA si installa solo in HTTPS.
